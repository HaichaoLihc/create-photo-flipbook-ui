...
class FilmStripArrangement {
  constructor({ book, pages, canStart, onBusyChange, onPhotoTap }) {
    this.book = book;
    ...
    for (const type of ['mousemove', 'touchmove', 'mouseup', 'touchend']) {
      document.addEventListener(type, event => {
        if (this.photoTap && (type === 'mouseup' || type === 'touchend')) {
          const frame = this.photoTap; this.photoTap = null;
          event.stopPropagation(); this.onPhotoTap?.(frame, event);
        } else if (this.ownsStripGesture) event.stopPropagation();
      }, { capture: true, passive: true });
    }
  ...
  pointerDown(event) {
    ...
    if (!handle) {
      this.pageGesture = { pointerId: event.pointerId, x: event.clientX, y: event.clientY, moved: false,
        photo: event.target.closest?.('button.negative-frame') };
  ...
  pointerUp(event) {
    if (this.pageGesture?.pointerId === event.pointerId) {
      ...
      if (gesture.moved) this.clickDeadline = performance.now() + 400;
      this.pageGesture = null;
      // Stationary photos own the following release, including at page corners.
      // A moved pointer still belongs entirely to the native flip renderer.
      if (!gesture.moved && gesture.photo) this.photoTap = gesture.photo;
  ...
  keyDown(event) {
    if (event.target.closest?.('dialog, input, textarea, select, [contenteditable]')) return;
